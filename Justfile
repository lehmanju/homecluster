...
@genconfig:
  ...
  talosctl gen config homecluster https://10.10.1.20:6443 --with-secrets ./clusterconfig/.secrets.yaml --config-patch-control-plane "@./minipc.config.yaml" --output-types controlplane,talosconfig --output-dir ./clusterconfig --force
  rm -f ./clusterconfig/.secrets.yaml
  talosctl --talosconfig ./clusterconfig/talosconfig config endpoint 10.10.1.22
  talosctl --talosconfig ./clusterconfig/talosconfig config node 10.10.1.22
...
@apply-initial: genconfig
  talosctl apply-config -n 10.10.1.22 --file homecluster-minipc.yaml --insecure
  # todo different nodes

[working-directory: 'talos/clusterconfig']  
@apply-update: genconfig
  talosctl apply-config -n 10.10.1.22 --file homecluster-minipc.yaml --talosconfig=./talosconfig
  # todo different nodes

@bootstrap:
  talosctl bootstrap -n 10.10.1.22 --talosconfig=./talos/clusterconfig/talosconfig
...
[working-directory: 'talos/clusterconfig']
@dashboard:
  talosctl -n 10.10.1.22 --talosconfig=./talosconfig dashboard
...
[working-directory: 'talos/clusterconfig']
@do +AARGS:
  talosctl -n 10.10.1.22 --talosconfig=./talosconfig {{ AARGS }}
